    ...
    public struct Color : IEquatable<Color> {
        ...
        public static Color FromHex(uint hex) {
            return new Color(
                ...
                a: hex > 0xffffff ? byte.MaxValue
                    : (byte)((hex >> 24) & byte.MaxValue));
        }

        private static float ToRange(in byte value) => value / (float)byte.MaxValue;
        private static void FromRange(in float range, out byte val) {
            val = (byte)Maths.Clamp((int)(range * byte.MaxValue), 0, byte.MaxValue);
        }
        ...
        public static explicit operator Color(Vector3 v) => new Color(v.X, v.Y, v.Z);
        public static explicit operator Vector3(Color c) => new Vector3(
            ToRange(c.R),
            ToRange(c.G),
            ToRange(c.B));

        public static explicit operator Color(Vector4 v) => new Color(v.X, v.Y, v.Z, v.W);
        public static explicit operator Vector4(Color c) => new Vector4(
            ToRange(c.R),
            ToRange(c.G),
            ToRange(c.B),
            ToRange(c.A));
        ...
        public override int GetHashCode() {
            ...
            int code = 1960784236;
            code = code * n + R.GetHashCode();
            code = code * n + G.GetHashCode();
            code = code * n + B.GetHashCode();
